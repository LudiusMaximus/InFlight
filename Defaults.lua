...
InFlight.defaults = {
  ...
  global = {
    ["Horde"] = {
      ...
      ["Reaver's Fall"] = {   -- Flightpath started by gossip option.
        ["Spinebreaker Post"] = 46,
      },
...
-- Make these available for export.
addon.global_classic = global_classic
addon.global_tbc = global_tbc
addon.global_wrath = global_wrath


-- Apply an era table on top of the post-Cataclysm defaults.
-- Merging per destination instead of replacing the whole source node keeps the
-- entries of the older eras that this table does not know about.
local function ApplyEraTable(eraNodes)
  for faction, factionNodes in pairs(eraNodes) do
    local defaultsFaction = InFlight.defaults.global[faction]
    if defaultsFaction then
      for sourceNode, destNodes in pairs(factionNodes) do
        defaultsFaction[sourceNode] = defaultsFaction[sourceNode] or {}
        for destNode, flightTime in pairs(destNodes) do
          defaultsFaction[sourceNode][destNode] = flightTime
        end
      end
    end
  end
...
-- Oldest era first, so that the closest matching era wins for each flight path.
ApplyEraTable(global_classic)
if buildInterface >= 20000 then
  ApplyEraTable(global_tbc)
end
if buildInterface >= 30000 then
  ApplyEraTable(global_wrath)
end
